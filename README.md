...
- 按分类展示常用链接，浅色四列卡片布局，移动端自动调整
- 本地搜索站点名称、分类和网址；`/` 聚焦搜索，`Esc` 清空
- 本地站点图标，未提供图标的网站显示首字
- 自动统计每个分类的链接数量
- 默认亮色，支持亮色与暗色主题；保留已有主题偏好
- 记住用户的主题选择
- 响应式网格布局，适配桌面与移动设备
...
网址分类和链接都位于 `index.html` 的 `.portals` 区域。新增或删除链接后，分类右上角的数量会自动更新。
...
- 两个出口地址不同则提示检测到不同出口；相同则提示未检测到分流。
